<script lang="ts">
  ...
  let progressRatio = $derived(machine.status === 'Available' || machine.status === 'Completed' || machine.estimatedComplete
...
<style>
  .machine { --machine-color: var(--wash); position: relative; min-width: 0; background: var(--panel); border: 1px solid transparent; border-radius: var(--radius-machine); transition: border-color .15s ease; }
  .machine.dry { --machine-color: var(--dry); }
  .machine:hover, .machine:focus-within { border-color: transparent; z-index: 2; }
