...
<style>
  ...
  .verification { position: relative; width: 300px; max-width: 100%; height: 65px; overflow: hidden; border-radius: var(--radius-control); background: var(--turnstile-surface); }
  .verification::after { content: ''; position: absolute; inset: 0; border: 1px solid var(--turnstile-surface); border-radius: inherit; pointer-events: none; }
